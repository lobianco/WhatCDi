...
- (id)initWithScrollView:(UIScrollView *)scrollView delegate:(id<SSPullToRefreshViewDelegate>)delegate {
    CGSize rippySize = [RippyContentView rippyImage].size;
	//CGRect frame = CGRectMake(0.0f, 0.0f - rippySize.height, scrollView.bounds.size.width, rippySize.height);
    CGRect frame = CGRectMake(0.0f, 0.0f - scrollView.frame.size.height, scrollView.bounds.size.width, scrollView.frame.size.height);
	if ((self = [self initWithFrame:frame])) {
		self.autoresizingMask = UIViewAutoresizingFlexibleWidth;
		self.scrollView = scrollView;
		self.delegate = delegate;
		self.state = SSPullToRefreshViewStateNormal;
		self.expandedHeight = rippySize.height;
        ...
        //add background
        UIImageView *backgroundView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"../Images/rippyBackgroundView.png"]];
        backgroundView.frame = CGRectMake(0, 0.0f - (backgroundView.image.size.height - scrollView.bounds.size.height), scrollView.bounds.size.width, backgroundView.image.size.height);
...
- (void)_setState:(SSPullToRefreshViewState)state animated:(BOOL)animated expanded:(BOOL)expanded completion:(void (^)(void))completion {
    ...
    CGFloat delay = 0.0;
    if (state == SSPullToRefreshViewStateClosing)
        delay = 0.3;
